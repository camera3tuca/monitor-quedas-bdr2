...
def buscar_fundamentalista_individual(ticker_bdr):
    """
    Busca dados fundamentalistas de um único ticker
    Usada em paralelo pela função batch
    """
    try:
...
def buscar_fundamentalistas_batch(tickers_bdr, max_workers=10):
    ...
    # Estatísticas
    yahoo_count = sum(1 for v in resultados.values() if v.get('fonte') == 'Yahoo Finance')
    brapi_count = sum(1 for v in resultados.values() if v.get('fonte') == 'BRAPI')
    
    st.success(f"""
    ✅ **Dados fundamentalistas:** {sucessos}/{total} BDRs
    - 📊 Yahoo Finance: {yahoo_count} (completo)
    - 📡 BRAPI: {brapi_count} (limitado)
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if not df_res.empty:
        ...
        evento = st.dataframe(
            ...
            column_order=("Ticker", "Empresa", "Fund_Score", "Liquidez", "Preco", "Queda_Dia", "IS", "Volume", "Gap", "Potencial", "Score", "Sinais"),
            column_config={
        ...
        if evento.selection and evento.selection.rows:
            ...
            if fund_data and fund_data.get('sucesso'):
                ...
                # Fonte dos dados
                if 'BRAPI' in fonte:
                    st.warning(f"⚠️ **Fonte:** {fonte} | Ticker: **{ticker_fonte}**")
                    st.info("""
                    📊 **Dados Limitados - Fonte BRAPI (B3)**
                    
                    A BRAPI fornece apenas dados básicos da BDR negociada na B3:
                    - ✅ Market Cap (capitalização de mercado)
                    - ✅ Volume de negociação na B3
                    - ❌ P/E Ratio (não disponível)
                    - ❌ Dividend Yield (não disponível)
                    - ❌ Crescimento de Receita (não disponível)
                    - ❌ Recomendação de Analistas (não disponível)
                    
                    **Por que isso acontece?**
                    - O Yahoo Finance não tem dados do ticker US mapeado
                    - A empresa pode não divulgar dados públicos completos
                    - O ticker US pode estar incorreto no mapeamento
                    
                    **Score calculado:** Baseado apenas em Market Cap e Volume (máximo 80 pontos).
                    """)
                else:
                    st.success(f"✅ **Fonte:** {fonte} | Ticker US: **{ticker_fonte}**")
                    st.info("📊 **Dados Completos** obtidos da empresa mãe (US) via Yahoo Finance")
