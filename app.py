    ...
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200:
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                if filtrar_ema20:
                    if 'EMA20' in df_ticker.columns:
                        ultima_ema20 = df_ticker['EMA20'].iloc[-1]
                        passa_ema20 = ultimo_close > ultima_ema20
                    else:
                        passa_ema20 = False
                
                if filtrar_ema50:
                    if 'EMA50' in df_ticker.columns and len(df_ticker) >= 50:
                        ultima_ema50 = df_ticker['EMA50'].iloc[-1]
                        passa_ema50 = ultimo_close > ultima_ema50
                    else:
                        passa_ema50 = False
                
                if filtrar_ema200:
                    if 'EMA200' in df_ticker.columns and len(df_ticker) >= 200:
                        ultima_ema200 = df_ticker['EMA200'].iloc[-1]
                        passa_ema200 = ultimo_close > ultima_ema200
                    else:
                        passa_ema200 = False
                
                # Adicionar se passar em todos os filtros selecionados
                if passa_ema20 and passa_ema50 and passa_ema200:
        ...
        if df_res_filtrado:
            ...
            filtros_ativos = []
            if filtrar_ema20:
                filtros_ativos.append("EMA20")
            if filtrar_ema50:
                filtros_ativos.append("EMA50")
            if filtrar_ema200:
                filtros_ativos.append("EMA200")
            
            st.info(f"📈 {len(df_res)} BDRs encontradas acima de: {', '.join(filtros_ativos)}")
        else:
            st.warning("⚠️ Nenhuma BDR encontrada com os filtros selecionados")
