...
# Importar OpenBB para dados fundamentalistas (fonte única e confiável)
try:
    from openbb import obb
    OPENBB_DISPONIVEL = True
except ImportError:
    OPENBB_DISPONIVEL = False
...
def buscar_fundamentalista_individual(ticker_bdr):
    """
    Busca dados fundamentalistas usando APENAS OpenBB
    Mais rápido e confiável - fonte única
    """
    if not OPENBB_DISPONIVEL:
        return ticker_bdr, {'sucesso': False}
    
    try:
        ticker_us = mapear_ticker_us(ticker_bdr)
        
        # Buscar perfil da empresa com OpenBB
        profile = obb.equity.profile(ticker_us)
        
        if not profile or not hasattr(profile, 'results') or len(profile.results) == 0:
            return ticker_bdr, {'sucesso': False, 'ticker_us': ticker_us}
        
        p = profile.results[0]
        
        # Dados básicos do perfil
        dados = {
            'marketCap': getattr(p, 'market_cap', None),
            'sector': getattr(p, 'sector', 'N/A'),
            'industry': getattr(p, 'industry', 'N/A'),
            'symbol': ticker_us,
        }
        
        # Buscar métricas fundamentalistas
        try:
            metrics = obb.equity.fundamental.metrics(ticker_us, limit=1)
            if metrics and hasattr(metrics, 'results') and len(metrics.results) > 0:
                m = metrics.results[0]
                dados['trailingPE'] = getattr(m, 'pe_ratio', None)
                dados['forwardPE'] = getattr(m, 'forward_pe', None)
                dados['dividendYield'] = getattr(m, 'dividend_yield', None)
                dados['revenueGrowth'] = getattr(m, 'revenue_growth', None)
        except:
            dados['trailingPE'] = None
            dados['dividendYield'] = None
            dados['revenueGrowth'] = None
        
        # Verificar se tem dados mínimos
        if not (dados.get('marketCap') or dados.get('trailingPE')):
            return ticker_bdr, {'sucesso': False, 'ticker_us': ticker_us}
...
def buscar_fundamentalistas_batch(tickers_bdr, max_workers=10):
    ...
    if not OPENBB_DISPONIVEL:
        st.error("""
        ❌ **OpenBB não está instalado**
        
        Para obter dados fundamentalistas, instale:
        ```bash
        pip install openbb
        ```
        
        Adicione ao requirements.txt: `openbb`
        """)
        return {}
    
    openbb_count = sum(1 for v in resultados.values() if v.get('fonte') == 'OpenBB')
    
    st.success(f"""
    ✅ **Dados fundamentalistas (OpenBB):** {sucessos}/{total} BDRs
    - 🔥 OpenBB: {openbb_count}
