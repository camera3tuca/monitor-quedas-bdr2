...
if st.button("🔄 Atualizar Análise", type="primary"):
    ...
    with st.spinner("Analisando oportunidades..."):
        ...
        if oportunidades:
            ...
            # Criar abas com os tickers
            num_graficos = min(10, len(df_res))  # Mostrar até 10 gráficos
            top_tickers = df_res.head(num_graficos)
            
            # Criar labels para as abas (Ticker + Queda%)
            tab_labels = [
                f"{row['Ticker']} ({row['Queda_Dia']:.1f}%)" 
            ...
            # Preencher cada aba com seu gráfico
            for i, (tab, (_, row)) in enumerate(zip(tabs, top_tickers.iterrows())):
                with tab:
                    ticker = row['Ticker']
                    try:
                        df_ticker = df_calc.xs(ticker, axis=1, level=1).dropna()
