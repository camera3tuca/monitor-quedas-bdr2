...
def buscar_fundamentalistas_batch(tickers_bdr):
    """
    Busca dados fundamentalistas de TODAS as BDRs em batch
    ...
    progress_text.text(f"📊 Buscando dados fundamentalistas... 0/{total}")
    
    for i, ticker_bdr in enumerate(tickers_bdr):
        try:
            ticker_us = mapear_ticker_us(ticker_bdr)
            
            # Tentar Yahoo Finance primeiro (mais rápido)
            try:
                ...
                if info and 'symbol' in info:
                    ...
                    if tem_dados:
                        ...
                        progress_text.text(f"📊 Buscando dados fundamentalistas... {i+1}/{total} (✅ {ticker_bdr})")
                        continue
            except Exception:
                pass
            
            # Se Yahoo falhou, tentar BRAPI como fallback
            try:
                ...
                if dados_brapi and dados_brapi.get('market_cap'):
                    ...
                    progress_bar.progress((i + 1) / total)
                    progress_text.text(f"📊 Buscando dados fundamentalistas... {i+1}/{total} (⚠️ {ticker_bdr})")
                    continue
            except Exception:
                pass
            
            # Ambas falharam - sem dados
            resultados[ticker_bdr] = {
                'sucesso': False,
            ...
            resultados[ticker_bdr] = {'sucesso': False}
            continue
    ...
    # Estatísticas
    total_sucesso = sum(1 for v in resultados.values() if v.get('sucesso'))
    ...
    st.success(f"""
    ✅ **Dados fundamentalistas carregados:** {total_sucesso}/{total} BDRs
    - 📊 Yahoo Finance: {yahoo_count} (dados completos)
    - 📡 BRAPI: {brapi_count} (dados limitados)
    - ❌ Sem dados: {total - total_sucesso}
    """)
...
if st.button("🔄 Atualizar Análise", type="primary"):
    ...
    # Buscar dados fundamentalistas de TODAS as BDRs
    st.markdown("### 📊 Carregando Dados Fundamentalistas")
    with st.spinner("Buscando dados fundamentalistas de todas as BDRs..."):
        # Pegar apenas BDRs que têm dados técnicos
        tickers_com_dados = df_calc.columns.get_level_values(1).unique().tolist()
        dados_fundamentalistas = buscar_fundamentalistas_batch(tickers_com_dados)
        
    # Analisar oportunidades COM dados fundamentalistas
