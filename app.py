    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            if fund_data:
                ...
                st.info(f"💡 Dados da empresa mãe (ticker US): **{ticker_us}**")
                ...
                with col3:
                    ...
                    rec = fund_data.get('recomendacao', 'N/A')
                    rec_map = {
                    ...
                    rec_texto, rec_cor = rec_map.get(rec, (rec.upper() if rec != 'N/A' else 'N/A', 'gray'))
                ...
                **Possíveis causas:**
                1. A BDR não tem ticker US correspondente no nosso mapeamento
                2. O ticker US não tem dados disponíveis no Yahoo Finance
                3. Erro temporário de conexão com a API
                4. A empresa não divulga dados fundamentalistas públicos
                
                **Solução:** Este ticker provavelmente não possui dados fundamentalistas completos disponíveis.
                """)
