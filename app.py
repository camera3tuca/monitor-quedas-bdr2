...
# yfinance já está importado acima - não precisa de bibliotecas adicionais
# Funciona perfeitamente no Streamlit Cloud sem problemas de permissão
...
def buscar_fundamentalista_individual(ticker_bdr):
    """
    Busca dados fundamentalistas usando yfinance otimizado
    Compatível com Streamlit Cloud - sem problemas de permissão
    """
    try:
...
def buscar_fundamentalistas_batch(tickers_bdr, max_workers=10):
    ...
    # Estatísticas
    # Estatísticas com yfinance
    if False:  # yfinance sempre disponível
        st.error("""
    ...
    yf_count = sum(1 for v in resultados.values() if v.get('fonte') == 'yFinance')
