...
def analisar_oportunidades(df_calc, mapa_nomes):
    ...
    for ticker in tickers:
        try:
            ...
            dias_disponiveis = min(20, len(df_ticker))
            volume_medio_20d = df_ticker['Volume'].tail(dias_disponiveis).mean()
            
            # Calcular frequência de gaps (diferença entre abertura e fechamento anterior)
            gaps = []
            for i in range(1, min(dias_disponiveis + 1, len(df_ticker))):
            ...
            frequencia_gaps = len(gaps)
            gap_medio = np.mean(gaps) if gaps else 0
            
            # Calcular dias com volume acima da média
            dias_volume_alto = sum(1 for v in df_ticker['Volume'].tail(dias_disponiveis) if v > volume_medio_20d * 0.8)
            
            # SCORE DE LIQUIDEZ (0-100)
            liquidez_score = 0
            
            # Volume médio (40 pontos) - Ajustado para ser mais generoso
            if volume_medio_20d > 500000:
                liquidez_score += 40
            elif volume_medio_20d > 100000:
                liquidez_score += 35
            elif volume_medio_20d > 50000:
                liquidez_score += 30
            elif volume_medio_20d > 10000:
                liquidez_score += 25
            elif volume_medio_20d > 5000:
                liquidez_score += 20
            elif volume_medio_20d > 1000:
                liquidez_score += 15
            elif volume_medio_20d > 100:
                liquidez_score += 10
            else:
                liquidez_score += 5
            
            # Frequência de gaps (30 pontos - quanto menos gaps, melhor)
            if frequencia_gaps == 0:
                liquidez_score += 30
            elif frequencia_gaps <= 2:
                liquidez_score += 25
            elif frequencia_gaps <= 5:
                liquidez_score += 20
            elif frequencia_gaps <= 8:
                liquidez_score += 15
            elif frequencia_gaps <= 12:
                liquidez_score += 10
            else:
                liquidez_score += 5
            
            # Consistência de volume (30 pontos)
            proporcao_volume = dias_volume_alto / dias_disponiveis if dias_disponiveis > 0 else 0
            if proporcao_volume >= 0.75:
                liquidez_score += 30
            elif proporcao_volume >= 0.5:
                liquidez_score += 20
            elif proporcao_volume >= 0.25:
                liquidez_score += 10
            else:
                liquidez_score += 5
            ...
            ranking_liquidez = max(0, min(10, ranking_liquidez))
...
def estilizar_liquidez_ranking(val):
    ...
    if pd.isna(val):
        return 'background-color: #e5e7eb; color: #6b7280; text-align: center;'
    
    ranking = int(val)
    
    # Degradê de vermelho (0) para verde (10) - Cores mais vibrantes
    if ranking == 10:
        return 'background: linear-gradient(135deg, #047857 0%, #059669 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 9:
        return 'background: linear-gradient(135deg, #059669 0%, #10b981 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 8:
        return 'background: linear-gradient(135deg, #10b981 0%, #34d399 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 7:
        return 'background: linear-gradient(135deg, #34d399 0%, #6ee7b7 100%); color: #064e3b; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 6:
        return 'background: linear-gradient(135deg, #6ee7b7 0%, #a7f3d0 100%); color: #064e3b; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 5:
        return 'background: linear-gradient(135deg, #fbbf24 0%, #fde047 100%); color: #78350f; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 4:
        return 'background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%); color: #78350f; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 3:
        return 'background: linear-gradient(135deg, #f97316 0%, #fb923c 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 2:
        return 'background: linear-gradient(135deg, #ef4444 0%, #f87171 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    elif ranking == 1:
        return 'background: linear-gradient(135deg, #dc2626 0%, #ef4444 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
    else:  # 0
        return 'background: linear-gradient(135deg, #991b1b 0%, #dc2626 100%); color: white; font-weight: 900; text-align: center; font-size: 1.2em;'
