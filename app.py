...
# Importar yahooquery para dados fundamentalistas
try:
    ...
    st.warning("""
    ⚠️ **YahooQuery não está instalado**
    ...
    Sem isso, os dados fundamentalistas não estarão disponíveis.
    """, icon="⚠️")
...
TICKER_MAP = {
    ...
    # Tech Giants
    'NFLX34': 'NFLX', 'SPOT34': 'SPOT', 'UBER34': 'UBER', 'LYFT34': 'LYFT',
    'AIRB34': 'ABNB', 'SNOW34': 'SNOW', 'DDOG34': 'DDOG', 'PANW34': 'PANW',
    
    # E-commerce & Retail  
    'MELI34': 'MELI', 'SHOP34': 'SHOP', 'EBAY34': 'EBAY', 'BKNG34': 'BKNG',
    'COWC34': 'COST', 'HOME34': 'HD', 'LOWC34': 'LOW', 'WALM34': 'WMT',
    
    # Entretenimento
    'DISB34': 'DIS', 'CMCS34': 'CMCSA', 'PARA34': 'PARA',
    
    # Semicondutores
    'INTC34': 'INTC', 'A1MD34': 'AMD', 'QCOM34': 'QCOM', 'AVGO34': 'AVGO',
    'ASML34': 'ASML', 'MUTC34': 'MU', 'TXIN34': 'TXN',
    
    # Software
    'ORCL34': 'ORCL', 'ADBE34': 'ADBE', 'CSCO34': 'CSCO', 'CTSH34': 'CTSH',
    'INTU34': 'INTC', 'S1NW34': 'NOW', 'CDAY34': 'CRM', 'WDAY34': 'WDAY',
    
    # Farmacêuticas
    'PFIZ34': 'PFE', 'MRCK34': 'MRK', 'ABBV34': 'ABBV', 'LILY34': 'LLY',
    'JNJB34': 'JNJ', 'BMYB34': 'BMY', 'GILD34': 'GILD', 'A1ZN34': 'AZN',
    
    # Financeiro
    'JPMC34': 'JPM', 'BOAC34': 'BAC', 'GSGI34': 'GS', 'MSBR34': 'MS',
    'CTGP34': 'C', 'CAON34': 'COF', 'S1CB34': 'SCHW',
    
    # Energia
    'EXXO34': 'XOM', 'CHVX34': 'CVX', 'COPH34': 'COP', 'SLBR34': 'SLB',
    'HALI34': 'HAL',
    
    # Industrial
    'BOEI34': 'BA', 'CATL34': 'CAT', 'GEOO34': 'GE', 'HOND34': 'HMC',
    'FDMO34': 'F', 'GMCO34': 'GM',
    
    # Telecom
    'ATTB34': 'T', 'VERI34': 'VZ', 'TMUS34': 'TMUS',
    
    # Consumo
    'COCA34': 'KO', 'PEPB34': 'PEP', 'MDLZ34': 'MDLZ', 'PMCO34': 'PM',
    'MOOO34': 'MO', 'NIKE34': 'NKE', 'SBUB34': 'SBUX',
    
    # Saúde
    'UNIH34': 'UNH', 'C1IC34': 'CI', 'CVSH34': 'CVS', 'H1UM34': 'HUM',
    
    # Chinesas
    'BABA34': 'BABA', 'JDCO34': 'JD', 'BIDU34': 'BIDU', 'TCOM34': 'TCOM',
    'N1IO34': 'NIO', 'P1DD34': 'PDD', 'B1IL34': 'BILI',
    
    # Outros
    'BERK34': 'BRK-B', 'VISA34': 'V', 'GOAU34': 'GOLD',
}

def mapear_ticker_us(ticker_bdr):
    """
    Mapeia BDR para ticker US com lógica melhorada
    1. Verifica dicionário manual
    2. Fallback: remove sufixo numérico
    """
    # Primeiro: tentar mapeamento manual
    if ticker_bdr in TICKER_MAP:
        return TICKER_MAP[ticker_bdr]
    
    # Fallback: remover sufixo (funciona para maioria)
    # Ex: AAPL34 → AAPL, MSFT34 → MSFT
    ticker_base = ticker_bdr.rstrip('0123456789')
    
    # Alguns casos especiais do fallback
...
def buscar_fundamentalista_individual(ticker_bdr):
    """
    Busca dados fundamentalistas usando YahooQuery
    Mais confiável e com API mais estável que yfinance
    """
    if not YAHOOQUERY_DISPONIVEL:
        return ticker_bdr, {'sucesso': False, 'erro': 'yahooquery_nao_instalado'}
    
    try:
        ticker_us = mapear_ticker_us(ticker_bdr)
        
        # Criar objeto YahooQuery
        yq = YQTicker(ticker_us)
        
        # Buscar múltiplos endpoints de uma vez (mais eficiente)
        ...
        if not summary or isinstance(summary, str):  # str = mensagem de erro
            return ticker_bdr, {
                'sucesso': False,
        ...
        # Verificar se tem pelo menos market cap ou P/E
        tem_dados = any([
            dados.get('marketCap'),
            dados.get('trailingPE'),
            dados.get('forwardPE'),
        ])
...
def buscar_fundamentalistas_batch(tickers_bdr, max_workers=10):
    ...
    if not YAHOOQUERY_DISPONIVEL:
        st.error("""
        ❌ **YahooQuery não instalado**
        
        Nenhum dado fundamentalista foi obtido.
    ...
    st.success(f"""
    ✅ **Dados fundamentalistas (YahooQuery):** {sucessos}/{total} BDRs
    - 📊 Dados obtidos: {sucessos}
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            if fund_data and fund_data.get('sucesso'):
                ...
                Este ticker não teve dados encontrados durante a análise inicial.
                
                **Possíveis causas:**
                - Ticker muito novo ou com baixo volume
                - Dados não disponíveis nas APIs consultadas
                - Ticker US incorreto no mapeamento
