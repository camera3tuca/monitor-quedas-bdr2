...
def calcular_score_fundamentalista(info):
    ...
    Adaptado do script fundamentalista
    """
    ...
    try:
        ...
        if pe:
            ...
            elif 5 <= pe < 10 or 25 < pe <= 35: score += 10
            elif pe < 5: score += 5
            elif pe > 50: score -= 10
        ...
        if div_yield:
            if div_yield > 0.04: score += 10
            elif div_yield > 0.02: score += 5
        ...
        elif rec == 'sell': score -= 5
        elif rec == 'strong_sell': score -= 10
        
        # Market Cap (10 pontos)
        mcap = info.get('marketCap')
        if mcap:
            if mcap > 1e12: score += 10
            elif mcap > 100e9: score += 5
...
def buscar_dados_fundamentalistas(ticker_bdr):
    """
    Busca dados fundamentalistas de forma segura
    ...
    try:
        ...
        score = calcular_score_fundamentalista(info)
        
        # Classificação
        if score >= 80: classif = "🌟"
        elif score >= 65: classif = "✅"
        elif score >= 50: classif = "⚖️"
        elif score >= 35: classif = "⚠️"
        else: classif = "🔴"
    ...
    for ticker in tickers:
        try:
            ...
            resultados.append({
                ...
                'Fund_Data': fund  # Dados completos para expander
            })
        except: continue
    return resultados
    ...
    if not df_res.empty:
        ...
        evento = st.dataframe(
            df_res.style.map(estilizar_potencial, subset=['Potencial'])
                        ...
                        .map(estilizar_fundamentalista, subset=['Fund_Class'])
            .format({
                'Preco': 'R$ {:.2f}',
                'Volume': '{:,.0f}',
                'Queda_Dia': '{:.2f}%',
            ...
            column_order=("Ticker", "Empresa", "Fund_Class", "Liquidez", "Preco", "Queda_Dia", "IS", "Volume", "Gap", "Potencial", "Score", "Sinais"),
            column_config={
                "Empresa": st.column_config.TextColumn("Empresa", width="medium"),
                "Fund_Class": st.column_config.TextColumn("📊 Fund", width="small",
                    help="Análise Fundamentalista: 🌟Excelente | ✅Bom | ⚖️Neutro | ⚠️Atenção | 🔴Evitar"),
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col2:
                    ...
                    if 'Fund_Data' in row and row['Fund_Data']:
                        fund = row['Fund_Data']
                        with st.expander("📊 Análise Fundamentalista"):
                            st.markdown(f"**Score:** {fund['score']:.0f}/100")
                            st.markdown(f"**Classificação:** {fund['classificacao']}")
                            
                            if fund.get('setor') and fund['setor'] != 'N/A':
                                st.markdown(f"**Setor:** {fund['setor']}")
                            
                            if fund.get('pe_ratio'):
                                st.markdown(f"**P/E Ratio:** {fund['pe_ratio']:.2f}")
                            
                            if fund.get('dividend_yield'):
                                st.markdown(f"**Dividend Yield:** {fund['dividend_yield']*100:.2f}%")
                            
                            if fund.get('market_cap'):
                                mcap_b = fund['market_cap'] / 1e9
                                st.markdown(f"**Market Cap:** ${mcap_b:.1f}B")
                            
                            if fund.get('revenue_growth'):
                                st.markdown(f"**Crescimento Receita:** {fund['revenue_growth']*100:.1f}%")
                            
                            rec = fund.get('recomendacao', 'N/A')
                            if rec and rec != 'N/A':
                                rec_map = {
                                ...
                                st.markdown(f"**Recomendação:** {rec_map.get(rec, rec)}")
