...
def analisar_oportunidades(df_calc, mapa_nomes):
    ...
    for ticker in tickers:
        try:
            ...
            if liquidez_score >= 80:
                classificacao_liquidez = "Excelente"
                cor_liquidez = "🟢"
            elif liquidez_score >= 60:
                classificacao_liquidez = "Boa"
                cor_liquidez = "🟢"
            elif liquidez_score >= 40:
                classificacao_liquidez = "Moderada"
                cor_liquidez = "🟡"
            else:
                classificacao_liquidez = "Baixa"
                cor_liquidez = "🔴"
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    # Linha 2: Filtro de Liquidez
    st.markdown("**💧 Filtro de Liquidez:**")
    col_liq1, col_liq2 = st.columns([1, 2])
    
    with col_liq1:
        filtrar_liquidez = st.checkbox(
            "🔵 Apenas Alta Liquidez",
            value=False,
            help="Mostrar somente BDRs com boa liquidez (score ≥ 60) - Menos gaps, volume consistente"
        )
    
    with col_liq2:
        if filtrar_liquidez:
            st.markdown("""
            <div style='background: #dbeafe; padding: 0.5rem; border-radius: 6px; border-left: 3px solid #3b82f6;'>
                <p style='margin: 0; color: #1e40af; font-size: 0.85rem;'>
                    ✓ Filtrando BDRs com liquidez ≥ 60 (Boa/Excelente)
                </p>
            </div>
            """, unsafe_allow_html=True)
    
    # Aplicar filtros se algum selecionado
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200 or filtrar_liquidez:
        df_res_filtrado = []
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                # Filtro de Liquidez
                if filtrar_liquidez and passa_filtro:
                    liquidez_score = opp.get('Liquidez_Score', 0)
                    if liquidez_score >= 60:  # Boa ou Excelente
        ...
        if df_res_filtrado:
            ...
            if filtrar_liquidez:
                filtros_ativos.append(f"Liquidez ({contadores['liquidez']} ✓)")
    ...
    if not df_res.empty:
        ...
        evento = st.dataframe(
            ...
            column_order=("Ticker", "Empresa", "Liquidez_Cor", "Liquidez_Class", "Preco", "Queda_Dia", "IS", "Volume", "Gap", "Potencial", "Score", "Sinais"),
            column_config={
                "Empresa": st.column_config.TextColumn("Empresa", width="medium"),
                "Liquidez_Cor": st.column_config.TextColumn("💧", width="small", help="Indicador de Liquidez"),
                "Liquidez_Class": st.column_config.TextColumn("Liquidez", width="small", help="Score de Liquidez: Excelente/Boa/Moderada/Baixa"),
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col2:
                    ...
                    liquidez_score = row.get('Liquidez_Score', 0)
                    
                    if liquidez_score >= 60:
                        bg_liquidez = "#d1fae5"
                        txt_liquidez = "#065f46"
                    elif liquidez_score >= 40:
                        bg_liquidez = "#fef3c7"
                    ...
                    <div style='background: {bg_liquidez}; padding: 0.75rem; border-radius: 6px; margin: 1rem 0;'>
                        <p style='margin: 0; font-weight: 600; color: {txt_liquidez}; font-size: 0.9rem;'>
                            {liquidez_cor} Liquidez: {liquidez_class}
                        </p>
                        <p style='margin: 0.3rem 0 0 0; color: {txt_liquidez}; font-size: 0.8rem;'>
                            Score: {liquidez_score:.0f}/100
                        </p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with st.expander("📊 Detalhes de Liquidez"):
                        ...
                        st.markdown("---")
                        if liquidez_score >= 80:
                            st.success("✅ Excelente liquidez! Baixo risco de dificuldade em executar ordens.")
                        elif liquidez_score >= 60:
                            st.success("✅ Boa liquidez! Risco moderado de gaps.")
                        elif liquidez_score >= 40:
                            st.warning("⚠️ Liquidez moderada. Atenção aos gaps e volume.")
                        else:
                            st.error("🔴 Baixa liquidez! Alto risco de gaps e dificuldade em executar ordens.")
