...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if filtrar_ema20 or filtrar_ema50 or filtrar_ema200:
        ...
        for opp in oportunidades:
            ...
            try:
                ...
                if filtrar_ema200 and passa_filtro:
                    if 'EMA200' in df_ticker.columns and tam >= 200:
        ...
        if df_res_filtrado:
            ...
            st.info(f"📈 {len(df_res)} BDRs passaram no filtro | {' + '.join(filtros_ativos)}")
        else:
            ...
            st.warning(f"⚠️ Nenhuma BDR passou em TODOS os filtros combinados")
            st.caption(f"Debug: {' | '.join(filtros_ativos)} | {contadores['sem_dados']} sem dados suficientes")
